...
def deploy():
   ...
   os.system("echo Atualizando sistema em: produção")
   run("cd /var/www/sic/ && git pull origin master && touch tmp/restart.txt")
...
def checkout_projeto():
   ...
   run("cd /var/www/sic/ && git fetch origin && git reset --hard origin/master && touch tmp/restart.txt")

def checkout_commit(commit):
   " => Executa checkout para commit informado. Ex: fab -f fab_producao.py checkout_commit:commit='commit'"
...
def executa_migrates(comandos):
   " => Executa migrates, argumento ou string vazia. Ex: fab -f fab_producao.py executa_migrates:migrate='comandos' ou 'string_vazia' "
